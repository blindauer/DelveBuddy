...
local function colorWithQuality(value, quality)
    ...
    if color and color.hex then
        return ("%s%s|r"):format(color.hex, tostring(value))
    end
...
local function addCrestLineToTooltip(prefix, reward)
    ...
    local crestIcon, crestName = getCrestIconAndName(reward)
    GameTooltip:AddLine(
        ("%s%s %s%s"):format(
            prefix or "",
...
local function ShowTierDetailTooltip(tier, rewards, rewardsTip, heroCrestInfo)
    GameTooltip:Hide()
    GameTooltip:SetOwner(rewardsTip, "ANCHOR_NONE")
    GameTooltip:ClearLines()
    GameTooltip:ClearAllPoints()
    local rewardsTipFrame = rewardsTip.frame or rewardsTip
    local rewardsTipPoint = select(1, rewardsTip:GetPoint(1))
    if rewardsTipPoint and rewardsTipPoint:find("RIGHT") then
        -- rewardsTip is right-anchored (extends left) → GameTooltip goes to its left
        GameTooltip:SetPoint("TOPRIGHT", rewardsTipFrame, "TOPLEFT", -8, 0)
    else
        -- rewardsTip is left-anchored (extends right) → GameTooltip goes to its right
        GameTooltip:SetPoint("TOPLEFT", rewardsTipFrame, "TOPRIGHT", 8, 0)
    end
    ...
    if rewards.nemesisCrest then
        local nemesisIcon, nemesisName = getCrestIconAndName(rewards.nemesisCrest)
        GameTooltip:AddLine(" ")
        GameTooltip:AddLine(
...
function DelveBuddy:ShowDelveRewardsTooltip(owner)
    ...
    rewardsTip:SetClampedToScreen(true)
    local charTip = self.charTip
    if charTip then
        -- Main tips are visible: read charTip's anchor to find which side the display is on,
        -- then position rewardsTip on the opposite side of the stack from the display.
        -- This mirrors the logic in OpenAllTips that stacks delveTip above/below charTip.
        local point = select(1, charTip:GetPoint(1))
        local ref = self.delveTip or charTip
        local refFrame = ref.frame or ref
        if point and point:find("RIGHT") then
            -- charTip's right edge is anchored → display is on the right → stack to the left
            rewardsTip:SetPoint("TOPRIGHT", refFrame, "TOPLEFT", -8, 0)
        else
            -- charTip's left edge is anchored → display is on the left → stack to the right
            rewardsTip:SetPoint("TOPLEFT", refFrame, "TOPRIGHT", 8, 0)
        end
    else
        -- Standalone (Shift+Click with no main tips open): owner is the actual display frame
        rewardsTip:SmartAnchorTo(owner)
    end
    ...
    for tier = 1, 11 do
        ...
        rewardsTip:SetLineScript(line, "OnEnter", function()
            ShowTierDetailTooltip(tier, rewards, rewardsTip, heroCrestInfo)
        end)
